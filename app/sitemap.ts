...
import { getPagesSlugByLanguageQuery, getPostsSlugByLanguageQuery } from "@/sanity/lib/queries";
import { SanityDocument } from "next-sanity";

const publicURL = process.env.NEXT_PUBLIC_SITE_URL;

function generatePagesSlugs(language: string) {
    return sanityFetch<SanityDocument>({query: getPagesSlugByLanguageQuery, params: {language}})
}

function generatePostsSlugs(language: string) {
    return sanityFetch<SanityDocument>({query: getPostsSlugByLanguageQuery, params: {language}})
}

export default async function Sitemap(): Promise<{ url: string; lastModified?: string | Date | undefined; changeFrequency?: "always" | "hourly" | "daily" | "weekly" | "monthly" | "yearly" | "never" | undefined; priority?: number | undefined; }[]> {
